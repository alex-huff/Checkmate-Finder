...
public class Board {
    ...
    public Board(Team turn, BoardPos enTar, boolean canWhiteQueenCastle, boolean canWhiteKingCastle, boolean canBlackQueenCastle, boolean canBlackKingCastle) {
        this.turn = turn;
        this.enTar = enTar;
    ...
    public boolean canQueenSideCastle(Team team) {
    ...
    public boolean canKingSideCastle(Team team) {
        if (team.equals(Team.WHITE)) {
    ...
    public void setPieceAt(int row, int col, Piece piece) {
        if (this.inBounds(row, col)) {
            ...
            if (piece != null) piece.updatePos(row, col);
        }
    }

    public BoardPos getEnTar() {
        return this.enTar;
    }

    public void setPieceAt(BoardPos pos, Piece piece) {
    ...
    public Piece getPieceAt(BoardPos pos) {
        return this.getPieceAt(pos.getRow(), pos.getCol());
    }

    public boolean tileInAttack(int row, int col, Team team) { //returns true of a tile is under attack by a piece of the opposite team
        for (int r = 0; r < 8; r++) {
            for (int c = 0; c < 8; c++) {
    ...
    public boolean tileInAttack(BoardPos pos, Team team) {
        return this.tileInAttack(pos.getRow(), pos.getCol(), team);
    }

    private boolean isCheckMove(Move move) {
    ...
    public void addMoveIfNoCheck(List<Move> moves, Move move) {
        Team team = this.turn;
    ...
    private List<Move> getAllCheckMoves() {
        List<Move> moves = new ArrayList<>();
    ...
    public long numMoves(int maxDepth) {
        return numMoves(0, maxDepth);
    }

    public Move getForceMateMove(int maxDepth, int checkDepth) {
        List<Move> moves;
    ...
    public List<Move> getForceMateMoves(int maxDepth, int checkDepth) {
        List<Move> moves;
    ...
    public boolean canForceMate(int depth, int maxDepth, int checkDepth) { //can attacking team force the mate
        if (depth == maxDepth) { //path has escape
            return false;
        }

        if (depth % 2 == 1) { //get out of check
            ...
            return true;
        } else { //only want moves that put in check
    ...
    public long numMoves(int depth, int maxDepth) {
        if (depth == maxDepth) {
            return 1;
        }
    ...
    private void restore(Piece piece) {
        if (piece != null) this.setPieceAt(piece.getPos(), piece);
    }
    ...
    public void reverseMove(Move move) {
        ...
        if (move.joint != null) { //only castling right now
    ...
    private void executeMove(Move move) {
        ...
        this.states.push(new BoardState(lastEnTar, oldPiece, taken, lastCanWhiteQueenCastle, lastCanWhiteKingCastle, lastCanBlackQueenCastle, lastCanBlackKingCastle));
        ...
        if (toMove instanceof PiecePawn) { //en passant target square
    ...
    private boolean isInCheck(Team team) {
        Piece king = null;
    ...
    public static Board getBoardFromFEN(String fen, boolean mirrored) {
        Board board;
        ...
        board = new Board(team, enPas, canWhiteQueenCastle, canWhiteKingCastle, canBlackQueenCastle, canBlackKingCastle);
