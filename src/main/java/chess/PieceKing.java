...
class PieceKing extends Piece
{
    ...
    private static Offset[] offsets = { //1 space in each direction
                                        new Offset(-1, 1), new Offset(0, 1), new Offset(1, 1), new Offset(-1, 0),
                                        new Offset(1, 0), new Offset(-1, -1), new Offset(0, -1), new Offset(1, -1),
                                        };

    public
    PieceKing(Board board, Team team)
    {
        super(board, team);
    }
    ...
    { // castling
        ...
        if (this.board.canQueenSideCastle(this.getTeam()) || this.board.canKingSideCastle(this.getTeam()))
        { // can't castle if not first move for king
            if (!this.board.tileInAttack(initialRow, 4, this.getTeam()))
            { // can't start in check
                List<Move> moves = new ArrayList<>();

                if (this.board.canQueenSideCastle(this.getTeam()))
                { // left is rook and has not moved
                    if (this.board.getPieceAt(initialRow, 1) == null && this.board.getPieceAt(initialRow, 2) == null &&
                        this.board.getPieceAt(initialRow, 3) == null)
                    {
                        ...
                        {
                            // tileInAttack is adequate because both king and rook, the only pieces that are moving,
                            // cannot be blocking a piece by nature
                ...
                if (this.board.canKingSideCastle(this.getTeam()))
                { // right is rook and has not moved
                    if (this.board.getPieceAt(initialRow, 6) == null && this.board.getPieceAt(initialRow, 5) == null)
                    {
                        // if nothing in between king and rook
